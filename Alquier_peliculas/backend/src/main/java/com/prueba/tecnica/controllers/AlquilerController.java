...
public class AlquilerController {
    ...
    public ResponseEntity<?> crearAlquiler(@RequestBody AlquilerDTO alquiler){
        ...
        if(cliente.isPresent()){
            ...
            alquilerBD.setFechaEntrega(null);
            alquilerBD.setFechaInicio(new Date());

            return ResponseEntity.ok(alquilerService.save(alquilerBD));
        }else {
            throw  new ClienteInexistenteException("La cedula digitada no coincide con la de algun cliente registrado.");
        }
    }

    @PutMapping
    public ResponseEntity<?> updateEstadoAlquiler( @RequestBody AlquilerDTO devolucion){

        return ResponseEntity.ok().body(alquilerService.ingresarDevolucion(devolucion));
    }
}
